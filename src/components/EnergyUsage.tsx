...
export default function EnergyUsage({ today, yesterday }: { today: StatisticEntry; yesterday: StatisticEntry }) {
    ...
    return (
        <UIBlock className="grid grid-cols-[auto_auto_1fr] grid-rows-[auto_auto] items-center min-w-full">
            ...
            <p className="text-3xl text-right tracking-wide">{today.kwh_usage.toFixed(1).replace('.', ',')} <span className="text-xs text-gray">kWh</span></p>
            {percentage !== 0 && <p className={`text-sm tracking-wide text-right ${percentage < 0 ? 'text-success' : 'text-red-600'}`}>{Math.abs(percentage)}% {percentage > 0 ? 'mere' : 'mindre'} end i går</p>}
        </UIBlock>
    )
}